...
from app.models import Project, Task, TaskExecution
...
class StatisticsService:
    ...
    def get_execution_trend(db: Session, days: int = 7) -> Dict:
        ...
        return {"dates": dates, "success": success_data, "failed": failed_data}

    @staticmethod
    def get_project_ranking(db: Session, limit: int = 10) -> List[Dict]:
        """获取项目活跃度排行"""
        stats = (
            db.query(Project.name, func.count(TaskExecution.id).label("count"))
    ...
    def get_status_distribution(db: Session) -> List[Dict]:
        """获取任务状态分布"""
        stats = (
            db.query(TaskExecution.status, func.count(TaskExecution.id).label("count"))
