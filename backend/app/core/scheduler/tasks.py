...
async def clean_stale_executions() -> None:
    ...
    try:
        ...
        stale_pending = db.query(TaskExecution).filter(
            TaskExecution.status == "pending",
            TaskExecution.start_time < now - pending_timeout
        ).all()
        ...
        stale_running = db.query(TaskExecution).join(Task).filter(
        ...
        for execution in stale_running:
            ...
            if execution.start_time and (now - execution.start_time).total_seconds() > timeout_seconds:
        ...
        if stale_pending or timeout_execution_ids:
            db.commit()
            logger.info(f"清理卡住的执行记录：{len(stale_pending)} 个 pending，{len(timeout_execution_ids)} 个 timeout")
